...
package demo.jbox2d.dynamics;

import demo.jbox2d.collision.*;
import demo.jbox2d.common.XForm;
import demo.jbox2d.dynamics.contacts.Contact;
import demo.jbox2d.dynamics.contacts.ContactEdge;
import demo.jbox2d.dynamics.joints.JointType;
...
public class World {
    ...
    demo.jbox2d.collision.BroadPhase m_broadPhase;
    ...
    demo.jbox2d.dynamics.joints.Joint m_jointList;
    ...
    demo.jbox2d.common.Vec2 m_gravity;
    ...
    demo.jbox2d.dynamics.DestructionListener m_destructionListener;
    BoundaryListener m_boundaryListener;
    demo.jbox2d.dynamics.ContactFilter m_contactFilter;
    demo.jbox2d.dynamics.ContactListener m_contactListener;
    DebugDraw m_debugDraw;

    private float m_inv_dt0;

    private ArrayList<demo.jbox2d.dynamics.Steppable> postStepList;
     ...
     */
    public World(demo.jbox2d.collision.AABB worldAABB, demo.jbox2d.common.Vec2 gravity, boolean doSleep) {
        m_positionCorrection = true;
        m_warmStarting = true;
        m_continuousPhysics = true;
        m_destructionListener = null;
        m_boundaryListener = null;
        m_contactFilter = demo.jbox2d.dynamics.ContactFilter.DEFAULT_FILTER;//&b2_defaultFilter;
        ...
        m_broadPhase = new demo.jbox2d.collision.BroadPhase(worldAABB, m_contactManager);

        BodyDef bd = new BodyDef();
        m_groundBody = createBody(bd);
        postStepList = new ArrayList<demo.jbox2d.dynamics.Steppable>();
    }
     ...
     */
    public demo.jbox2d.common.Vec2 getGravity() {
     ...
     */
    public void setGravity(demo.jbox2d.common.Vec2 gravity) {
        m_gravity = gravity;
    }
     ...
     */
    public demo.jbox2d.dynamics.joints.Joint getJointList() {
        return m_jointList;
    }
    ...
    public void destroyBody(Body b) {
        ...
        demo.jbox2d.dynamics.joints.JointEdge jn = b.m_jointList;
        while (jn != null) {
            demo.jbox2d.dynamics.joints.JointEdge jn0 = jn;
            jn = jn.next;
        ...
        demo.jbox2d.collision.Shape s = b.m_shapeList;
        while (s != null) {
            demo.jbox2d.collision.Shape s0 = s;
            s = s.m_next;
            ...
            demo.jbox2d.collision.Shape.destroy(s0);
        }
     ...
     * <BR><em>Warning</em> This function is locked during callbacks.
     */
    public demo.jbox2d.dynamics.joints.Joint createJoint(demo.jbox2d.dynamics.joints.JointDef def) {
        assert (m_lock == false);

        demo.jbox2d.dynamics.joints.Joint j = demo.jbox2d.dynamics.joints.Joint.create(def);
        ...
        if (def.collideConnected == false) {
            ...
            Body b = def.body1.m_shapeCount < def.body2.m_shapeCount ? def.body1
                    : def.body2;
            for (demo.jbox2d.collision.Shape s = b.m_shapeList; s != null; s = s.m_next) {
     ...
     * <BR><em>Warning</em>: This function is locked during callbacks.
     */
    public void destroyJoint(demo.jbox2d.dynamics.joints.Joint j) {
        assert (m_lock == false);
        ...
        demo.jbox2d.dynamics.joints.Joint.destroy(j);
    ...
    private void postStep(float dt, int iterations) {
        for (demo.jbox2d.dynamics.Steppable s : postStepList) {
     ...
     * @param s
     */
    public void registerPostStep(demo.jbox2d.dynamics.Steppable s) {
        postStepList.add(s);
    }
     ...
     */
    public void refilter(demo.jbox2d.collision.Shape shape) {
     ...
     */
    public demo.jbox2d.collision.Shape[] query(demo.jbox2d.collision.AABB aabb, int maxCount) {
        Object[] objs = m_broadPhase.query(aabb, maxCount);
        demo.jbox2d.collision.Shape[] ret = new demo.jbox2d.collision.Shape[objs.length];
    ...
    public void solve(TimeStep step) {
        ...
        for (demo.jbox2d.dynamics.joints.Joint j = m_jointList; j != null; j = j.m_next) {
        ...
        for (Body seed = m_bodyList; seed != null; seed = seed.m_next) {
            ...
            while (stackCount > 0) {
                ...
                for (demo.jbox2d.dynamics.joints.JointEdge jn = b.m_jointList; jn != null; jn = jn.next) {
    ...
    public void solveTOI(TimeStep step) {
        ...
        Island island = new Island(m_bodyCount, demo.jbox2d.common.Settings.maxTOIContactsPerIsland, 0, m_contactListener);
        int stackSize = m_bodyCount;
        Body[] stack = new Body[stackSize];
        ...
        while (true) {
            ...
            for (Contact c = m_contactList; c != null; c = c.m_next) {
                ...
                if ((c.m_flags & Contact.e_toiFlag) != 0) {
                    ...
                    demo.jbox2d.collision.Shape s1 = c.getShape1();
                    demo.jbox2d.collision.Shape s2 = c.getShape2();
                    Body b1 = s1.getBody();
                    Body b2 = s2.getBody();
                ...
                if (demo.jbox2d.common.Settings.EPSILON < toi && toi < minTOI) {
            ...
            if (minContact == null || 1.0f - 100.0f * demo.jbox2d.common.Settings.EPSILON < minTOI) {
                // No more TOI events. Done!
                break;
            }

            // Advance the bodies to the TOI.
            demo.jbox2d.collision.Shape s1 = minContact.getShape1();
            demo.jbox2d.collision.Shape s2 = minContact.getShape2();
            Body b1 = s1.getBody();
            Body b2 = s2.getBody();
            b1.advance(minTOI);
            b2.advance(minTOI);
            ...
            assert (subStep.dt > demo.jbox2d.common.Settings.EPSILON);
     ...
     * For internal use
     */
    public void drawShape(demo.jbox2d.collision.Shape shape, XForm xf, demo.jbox2d.common.Color3f color, boolean core) {
        demo.jbox2d.common.Color3f coreColor = new demo.jbox2d.common.Color3f(255f * 0.9f, 255f * 0.6f, 255f * 0.6f);

        if (shape.getType() == demo.jbox2d.collision.ShapeType.CIRCLE_SHAPE) {
            demo.jbox2d.collision.CircleShape circle = (CircleShape) shape;

            demo.jbox2d.common.Vec2 center = XForm.mul(xf, circle.getLocalPosition());
            float radius = circle.getRadius();
            demo.jbox2d.common.Vec2 axis = xf.R.col1;

            m_debugDraw.drawSolidCircle(center, radius, axis, color);

            if (core) {
                m_debugDraw.drawCircle(center, radius - demo.jbox2d.common.Settings.toiSlop, coreColor);
            }
        } else if (shape.getType() == demo.jbox2d.collision.ShapeType.POLYGON_SHAPE) {
            demo.jbox2d.collision.PolygonShape poly = (demo.jbox2d.collision.PolygonShape) shape;
            int vertexCount = poly.getVertexCount();
            demo.jbox2d.common.Vec2[] localVertices = poly.getVertices();

            assert (vertexCount <= demo.jbox2d.common.Settings.maxPolygonVertices);
            demo.jbox2d.common.Vec2[] vertices = new demo.jbox2d.common.Vec2[vertexCount];
            ...
            if (core) {
                demo.jbox2d.common.Vec2[] localCoreVertices = poly.getCoreVertices();
                for (int i = 0; i < vertexCount; ++i) {
     ...
     * For internal use
     */
    public void drawJoint(demo.jbox2d.dynamics.joints.Joint joint) {
        Body b1 = joint.getBody1();
        Body b2 = joint.getBody2();
        XForm xf1 = b1.getXForm();
        XForm xf2 = b2.getXForm();
        demo.jbox2d.common.Vec2 x1 = xf1.position;
        demo.jbox2d.common.Vec2 x2 = xf2.position;
        demo.jbox2d.common.Vec2 p1 = joint.getAnchor1();
        demo.jbox2d.common.Vec2 p2 = joint.getAnchor2();

        demo.jbox2d.common.Color3f color = new demo.jbox2d.common.Color3f(255f * 0.5f, 255f * 0.8f, 255f * 0.8f);
        ...
        if (type == JointType.DISTANCE_JOINT) {
            ...
            demo.jbox2d.dynamics.joints.PulleyJoint pulley = (demo.jbox2d.dynamics.joints.PulleyJoint) joint;
            demo.jbox2d.common.Vec2 s1 = pulley.getGroundAnchor1();
            demo.jbox2d.common.Vec2 s2 = pulley.getGroundAnchor2();
    ...
    public void drawDebugData() {
        ...
        if ((flags & DebugDraw.e_shapeBit) != 0) {
            ...
            for (Body b = m_bodyList; b != null; b = b.getNext()) {
                XForm xf = b.getXForm();
                for (demo.jbox2d.collision.Shape s = b.getShapeList(); s != null; s = s.getNext()) {
                    if (s.isSensor()) continue;
                    if (b.isStatic()) {
                        drawShape(s, xf, new demo.jbox2d.common.Color3f(255f * 0.5f, 255f * 0.9f, 255f * 0.5f), core);
                    } else if (b.isSleeping()) {
                        drawShape(s, xf, new demo.jbox2d.common.Color3f(255f * 0.5f, 255f * 0.5f, 255f * 0.9f), core);
                    } else {
                        drawShape(s, xf, new demo.jbox2d.common.Color3f(255f * 0.9f, 255f * 0.9f, 255f * 0.9f), core);
                    }
                }
            }
        }
        ...
        if ((flags & DebugDraw.e_jointBit) != 0) {
            for (demo.jbox2d.dynamics.joints.Joint j = m_jointList; j != null; j = j.getNext()) {
        ...
        if ((flags & DebugDraw.e_pairBit) != 0) {
            demo.jbox2d.collision.BroadPhase bp = m_broadPhase;
            demo.jbox2d.common.Vec2 invQ = new demo.jbox2d.common.Vec2(0.0f, 0.0f);
            invQ.set(1.0f / bp.m_quantizationFactor.x, 1.0f / bp.m_quantizationFactor.y);
            demo.jbox2d.common.Color3f color = new demo.jbox2d.common.Color3f(255f * 0.9f, 255f * 0.9f, 255f * 0.3f);

            for (int i = 0; i < demo.jbox2d.collision.PairManager.TABLE_CAPACITY; ++i) {
                int index = bp.m_pairManager.m_hashTable[i];
                while (index != PairManager.NULL_PAIR) {
                    Pair pair = bp.m_pairManager.m_pairs[index];
                    demo.jbox2d.collision.Proxy p1 = bp.m_proxyPool[pair.proxyId1];
                    demo.jbox2d.collision.Proxy p2 = bp.m_proxyPool[pair.proxyId2];

                    demo.jbox2d.collision.AABB b1 = new demo.jbox2d.collision.AABB();
                    demo.jbox2d.collision.AABB b2 = new demo.jbox2d.collision.AABB();
                    ...
                    demo.jbox2d.common.Vec2 x1 = new demo.jbox2d.common.Vec2(0.5f * (b1.lowerBound.x + b1.upperBound.x),
                            0.5f * (b1.lowerBound.y + b1.upperBound.y));
                    demo.jbox2d.common.Vec2 x2 = new demo.jbox2d.common.Vec2(0.5f * (b2.lowerBound.x + b2.upperBound.x),
        ...
        demo.jbox2d.common.Vec2 worldLower = bp.m_worldAABB.lowerBound;
        demo.jbox2d.common.Vec2 worldUpper = bp.m_worldAABB.upperBound;

        if ((flags & DebugDraw.e_aabbBit) != 0) {

            demo.jbox2d.common.Vec2 invQ = new demo.jbox2d.common.Vec2();
            invQ.set(1.0f / bp.m_quantizationFactor.x, 1.0f / bp.m_quantizationFactor.y);
            demo.jbox2d.common.Color3f color = new demo.jbox2d.common.Color3f(255f * 0.9f, 255f * 0.3f, 255f * 0.9f);
            for (int i = 0; i < demo.jbox2d.common.Settings.maxProxies; ++i) {
                Proxy p = bp.m_proxyPool[i];
                if (p.isValid() == false) {
                    continue;
                }

                demo.jbox2d.collision.AABB b = new demo.jbox2d.collision.AABB();
                ...
                demo.jbox2d.common.Vec2[] vs = new demo.jbox2d.common.Vec2[4];
                vs[0] = new demo.jbox2d.common.Vec2(b.lowerBound.x, b.lowerBound.y);
                vs[1] = new demo.jbox2d.common.Vec2(b.upperBound.x, b.lowerBound.y);
                vs[2] = new demo.jbox2d.common.Vec2(b.upperBound.x, b.upperBound.y);
                vs[3] = new demo.jbox2d.common.Vec2(b.lowerBound.x, b.upperBound.y);
        ...
        demo.jbox2d.common.Vec2[] vsw = new demo.jbox2d.common.Vec2[4];
        vsw[0] = new demo.jbox2d.common.Vec2(worldLower.x, worldLower.y);
        vsw[1] = new demo.jbox2d.common.Vec2(worldUpper.x, worldLower.y);
        vsw[2] = new demo.jbox2d.common.Vec2(worldUpper.x, worldUpper.y);
        vsw[3] = new demo.jbox2d.common.Vec2(worldLower.x, worldUpper.y);
        m_debugDraw.drawPolygon(vsw, 4, new demo.jbox2d.common.Color3f(255.0f * 0.3f, 255.0f * 0.9f, 255.0f * 0.9f));

        if ((flags & DebugDraw.e_obbBit) != 0) {
            demo.jbox2d.common.Color3f color = new demo.jbox2d.common.Color3f(0.5f, 0.3f, 0.5f);
            ...
            for (Body b = m_bodyList; b != null; b = b.getNext()) {
                ...
                for (Shape s = b.getShapeList(); s != null; s = s.getNext()) {
                    ...
                    demo.jbox2d.collision.PolygonShape poly = (PolygonShape) s;
                    OBB obb = poly.getOBB();
                    demo.jbox2d.common.Vec2 h = obb.extents;
                    demo.jbox2d.common.Vec2[] vs = new demo.jbox2d.common.Vec2[4];
                    vs[0] = new demo.jbox2d.common.Vec2(-h.x, -h.y);
                    vs[1] = new demo.jbox2d.common.Vec2(h.x, -h.y);
                    vs[2] = new demo.jbox2d.common.Vec2(h.x, h.y);
                    vs[3] = new demo.jbox2d.common.Vec2(-h.x, h.y);

                    for (int i = 0; i < 4; ++i) {
                        vs[i] = obb.center.add(demo.jbox2d.common.Mat22.mul(obb.R, vs[i]));
     ...
     */
    public demo.jbox2d.collision.AABB getWorldAABB() {
