...
public class MyBase64Encoder {
    ...
    static {
        int index = 0;
        for (int i = 0; i <= 25; i++) {
            INDEX_MAP.put(index, (char) ('A' + i));
            index++;
        }

        for (int j = 0; j <= 25; j++) {
            INDEX_MAP.put(index, (char) ('a' + j));
            index++;
        }

        for (int k = 0; k <= 9; k++) {
            INDEX_MAP.put(index, (char) ('0' + k));
            index++;
        }
    ...
    private static String convertByteArray2BinaryString(byte[] bytes) {
        ...
        int totalCount = paddingCount > 0 ? binaryBuilder.length() / 6 + 1 : binaryBuilder.length() / 6;
        int actualLength = 6 * totalCount;

        // 百分号后面的-号表示长度不够规定长度时，右填充。否则左填充。
        return String.format("%-" + actualLength + "s", binaryBuilder.toString()).replace(' ', '0');
    }

    private static String escapeBinaryString(String binaryString) throws Exception {
        if (null == binaryString || binaryString.isEmpty() || binaryString.length() % 6 != 0) {
    ...
    private static String paddingEscapeString(String escapeString) {
        ...
        int totalCount = paddingCount > 0 ? escapeString.length() / 4 + 1 : escapeString.length() / 4;
        int actualCount = 4 * totalCount;
        return String.format("%-" + actualCount + "s", escapeString).replace(' ', PADDING_CHAR);
    }
    ...
    private static String convertByte2BinaryString(byte b) {
        ...
        if (b >= 0) {
             ...
             * 会抛出异常。而Byte又没有提供toBinaryString的方法 为了保存byte的二进制值，可利用按位与的方法 例如有一个负数1000
             * 1111，要把它以字符串保留出来，利用它与1111 1111的与操作， 再转成int类型。1000 1111 & 1111 1111
             * 在内存中保存的就是 00000000 10001111，这时保存的是一个正整数。但我们不关心整数的正负，
             * 因为我们的目的是要把这串字符串截取出来 再利用Integer.toBinaryString()打印出来。
...
class MyBase64Decoder {
    ...
    private static String removePaddingChar(String base64String) {
        ...
        return firstPaddingIndex >= 0 ? base64String.substring(0, firstPaddingIndex) : base64String;
    }
    ...
    private static String getBinaryString(String base64String) {
        ...
        for (char c : base64String.toCharArray()) {
            ...
            binaryBuilder.append(String.format("%6s", Integer.toBinaryString(value)).replace(" ", "0"));
        }
    ...
    private static String removePaddingNumber(String binaryString) {
        ...
        binaryString = binaryString.substring(0, binaryString.length() - remainder);
    ...
    private static byte[] convertBinaryString2Bytes(String binaryString) {
        ...
        for (int i = 0; i <= binaryString.length() - 1; i += 8, arrayIndex++) {
            ...
            if (byteString.startsWith("0")) {
                ...
                /**
                 * 其他的负数，就要按照补码的规则来计算 即，原码取反+1=补码 那么，补码-1取反=原码 注意这都是真值部分的计算，符号位不能变
                 */
                // 补码
                 ...
                 * 若与1开头的异或，结果为1，而在int中该位不是符号位，会当成数值计算，造成数值错误。 因此，必须结果是0，即，两个0的异或。
                 */
                // 真值 8位的计算
