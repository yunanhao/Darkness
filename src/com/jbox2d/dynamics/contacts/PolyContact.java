...
import com.jbox2d.dynamics.ContactListener;
...
public class PolyContact extends Contact implements ContactCreateFcn {
    ...
    com.jbox2d.collision.Manifold m_manifold;

    public PolyContact(com.jbox2d.collision.Shape s1, com.jbox2d.collision.Shape s2) {
        super(s1, s2);
        assert (m_shape1.getType() == com.jbox2d.collision.ShapeType.POLYGON_SHAPE);
        assert (m_shape2.getType() == ShapeType.POLYGON_SHAPE);

        m_manifold = new com.jbox2d.collision.Manifold();
        m_manifoldCount = 0;
    }
    ...
    public Contact clone() {
        ...
        if (this.m_manifold != null) newC.m_manifold = new com.jbox2d.collision.Manifold(this.m_manifold);
    ...
    @Override
    public List<com.jbox2d.collision.Manifold> getManifolds() {
        // System.out.println("PolyContact.GetManifolds()");
        List<com.jbox2d.collision.Manifold> ret = new ArrayList<com.jbox2d.collision.Manifold>();
        if (m_manifold != null) {
            ret.add(m_manifold);
        }
    ...
    public Contact create(com.jbox2d.collision.Shape shape1, Shape shape2) {
        return new PolyContact(shape1, shape2);
    }

    public void dumpManifoldPoints() {
        for (int i = 0; i < m_manifold.pointCount; ++i) {
            com.jbox2d.collision.ManifoldPoint mp = m_manifold.points[i];
    ...
    public void evaluate(ContactListener listener) {
        com.jbox2d.dynamics.Body b1 = m_shape1.getBody();
        Body b2 = m_shape2.getBody();
        // Manifold m0 = m_manifold;
        com.jbox2d.collision.Manifold m0 = new Manifold(m_manifold);
        //This next stuff might be unnecessary now [ewj: nope, we need it]
        for (int k = 0; k < m_manifold.pointCount; k++) {
            m0.points[k] = new com.jbox2d.collision.ManifoldPoint(m_manifold.points[k]);
        ...
        CollidePoly.collidePolygons(m_manifold, (com.jbox2d.collision.PolygonShape) m_shape1, b1.getXForm(), (PolygonShape) m_shape2, b2.getXForm());

        boolean[] persisted = {false, false};

        com.jbox2d.dynamics.contacts.ContactPoint cp = new ContactPoint();
        cp.shape1 = m_shape1;
        cp.shape2 = m_shape2;
        ...
        if (m_manifold.pointCount > 0) {
            ...
            for (int i = 0; i < m_manifold.pointCount; ++i) {
                ...
                boolean found = false;
                com.jbox2d.collision.ContactID id = new com.jbox2d.collision.ContactID(mp.id);
                ...
                for (int j = 0; j < m0.pointCount; ++j) {
                    ...
                    com.jbox2d.collision.ManifoldPoint mp0 = m0.points[j];
                    ...
                    if (mp0.id.isEqual(id)) {
                        ...
                        if (listener != null) {
                            ...
                            cp.id = new com.jbox2d.collision.ContactID(id);
                            listener.persist(cp);
                        }
                        break;
                    }
